...
class ETLDiskJob(ProcessData):
    def __init__(self, bucket: str, minio_client: Any, path: str, save_image: Optional[bool], task: Optional[str], column: str,
                 model: str, bloom_filter: Optional[BloomFilter], folder_name: Optional[str], store_negative: Optional[bool]):
            ...
            for file in files:
                ...
                if not checked_obj:
                            ...
                            if not processed_df.empty:
                                ...
                                if self.task:
                                    if self.task ==  "multi-model":
                                        ...
                                        processed_df = self.perform_classification(processed_df, image_bucket)
                                if not self.store_negative and "label" in processed_df.columns:
                                    processed_df = processed_df[processed_df["label"]==1]
                                processed_df = self.run_extraction(processed_df)
                                processed_df = self.save_image_if_applicable(processed_df, date)
                                if not processed_df.empty:
                                    if self.minio_client:
                                        self.load_file_to_minio(final_filename, processed_df)
                                    else:
                    ...
                    if self.minio_client:
                        ...
                        image_bucket = f"images-{date}"
                        if self.task and "label" not in df.columns:
                            if self.task ==  "multi-model":
                                df = self.save_image_if_applicable(df, date)
                                processed_df = self.perform_classification(df, image_bucket)
                            else:
                                processed_df = self.perform_classification(df, image_bucket)
                            if not self.store_negative and "label" in processed_df.columns:
                                processed_df = processed_df[processed_df["label"]==1]
    ...
    def save_image_if_applicable(self, processed_df, date):
        if self.save_image and "image_path" not in processed_df.columns:
            if self.minio_client:
                image_bucket = f"images-{date}"
