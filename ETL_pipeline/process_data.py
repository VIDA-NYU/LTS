...
class ProcessData:
    ...
    def run_classification(self, df: pd.DataFrame, bucket_name: str) -> pd.DataFrame:
    ...
    def run_extraction(self, df: pd.DataFrame) -> pd.DataFrame:
        nu_extract = ExtractionJob()
        df = nu_extract.perform_extraction(df)
        return df
    ...
    def fix_price_str(price):
        ...
        if isinstance(price, str):
            ...
            if cleaned_price:
                extracted_value = float(cleaned_price.replace(",", "").replace(".", ""))
            ...
            for item in price:
                ...
                try:
                    ...
                    if cleaned_item:
                        extracted_value = float(cleaned_item.replace(",", "").replace(".", ""))
                        # Check if the extracted value has no fractional part
                        if extracted_value.is_integer():
                            extracted_value = int(extracted_value)
