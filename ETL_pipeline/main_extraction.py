...
def main():
    ...
    print("Starting regular extraction Job from Disk")
    path = f"/data/{filename}/data_pages/"
    folder_name = ""
    etl_job = ExtractionDisk(bucket=final_bucket, minio_client=minio_client, path=path, save_image=save_image,
                            task=task, column=column, model=model, bloom_filter=bloom)
    etl_job.run(folder_name=folder_name)
