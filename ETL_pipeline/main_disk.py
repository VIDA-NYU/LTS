...
def main():
    ...
    store_negative = True if args.storenegative else False
    ...
    if temporal:
        ...
        for file in files:
            ...
            etl_job = ETLDiskJob(bucket=final_bucket, minio_client=minio_client, path=path, save_image=save_image,
                                 task=task, column=column, model=model, bloom_filter=bloom, folder_name=folder_name, store_negative=store_negative)
...
def create_arg_parser():
    ...
    parser.add_argument('--storenegative', type=bool, required=False,
                        help='Store negative values if specified')
    parser.add_argument('--foldername', type=str, required=False,
                        help='Folder name for local data')
